...
class OutputItem(BaseModel):
    ...
    def serialize(self) -> dict[str, any]:
        ...
        return {
            k: (round(v, 2) if isinstance(v, float) else v if v is not None else "-")
...
class ComplianceSummaryModel(BaseModel):
    group_name: str | None = None
    non_compliant_members: list[int] | str | None = Field(default=None, description="List of non-compliant members or '-' if empty")
    compliance: str | None = Field(default=None, description="'Complies' if empty, 'Not Complies' otherwise")
    
    @validator('non_compliant_members', pre=True)
    def replace_empty_list(cls, value):
        if isinstance(value, list) and not value:
            return "-"
        return value

    @validator('compliance', always=True)
    def set_compliance_status(cls, v, values):
        if values.get('non_compliant_members') == "-":
            return "Complies"
        return "Not Complies"
...
class ComplianceSummaryList:
    ...
    def parse_from_dict(self, data: dict[str, list[int]]) -> None:
        ...
        self.items = [
            ComplianceSummaryModel(group_name=group, non_compliant_members=members)
...
class ConnectionSummaryList:
    ...
    def parse_from_dict(self, data: dict[str, str]) -> None:
        ...
        self.items = [
            ConnectionSummaryModel(group_name=group, connection_type=type_)
            for group, type_ in data.items()
        ]
